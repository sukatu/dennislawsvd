...
const CourtTypeManagement = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(`Failed to save court type: ${errorData.detail || 'Unknown error'}`);
      }
    } catch (error) {
      console.error('Error saving court type:', error);
      alert('Failed to save court type. Please try again.');
    } finally {
      setSaving(false);
    }
  };
  ...
  const handleDelete = async (courtType) => {
    if (!window.confirm(`Are you sure you want to delete ${courtType.name}?`)) {
      return;
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert('Failed to delete court type. Please try again.');
      }
    } catch (error) {
  ...
  const filteredCourtTypes = courtTypes.filter(courtType => {
    const matchesSearch = !searchTerm || 
      (courtType.name?.toLowerCase().includes(searchTerm.toLowerCase()) || false) ||
      (courtType.code?.toLowerCase().includes(searchTerm.toLowerCase()) || false) ||
      (courtType.description?.toLowerCase().includes(searchTerm.toLowerCase()) || false);
    
    const matchesLevel = !filterLevel || courtType.level === filterLevel;
    const matchesRegion = !filterRegion || 
      (courtType.region?.toLowerCase().includes(filterRegion.toLowerCase()) || false);
