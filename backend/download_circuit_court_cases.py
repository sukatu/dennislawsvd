#!/usr/bin/env python3
"""
Script to download Circuit Court cases from GhaLII
...
class CircuitCourtCaseDownloader:
    def __init__(self):
        self.base_url = "https://ghalii.org"
        self.circuit_court_url = "https://ghalii.org/judgments/GHACC/"
    ...
    def get_case_links(self, max_pages=50):
        """Get all case links from GhaLII Circuit Court"""
        case_links = []
        
        logger.info(f"Fetching case links from {self.circuit_court_url}")
        
        for page in range(1, max_pages + 1):
            try:
                # GhaLII uses pagination
                url = f"{self.circuit_court_url}?page={page}" if page > 1 else self.circuit_court_url
    ...
    def extract_case_details(self, case_url, case_title, court_type):
        ...
        try:
            ...
            if '/akn/gh/judgment/' in case_url:
                citation_match = re.search(r'\[(\d{4})\] GHACC (\d+)', case_title)
                if citation_match:
                    case_data['citation'] = f"[{citation_match.group(1)}] GHACC {citation_match.group(2)}"
    ...
    def download_cases(self, max_cases=100):
        """Download and save cases from GhaLII Circuit Court"""
        logger.info(f"Starting download of Circuit Court cases...")
        
        # Get case links
        case_links = self.get_case_links(max_pages=50)
...
def main():
    ...
    downloader = CircuitCourtCaseDownloader()
    
    try:
        # Download Circuit Court cases
        logger.info("Downloading Circuit Court cases...")
        circuit_success, circuit_failed = downloader.download_cases(max_cases=50)
